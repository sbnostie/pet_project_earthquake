...
with DAG(
    ...
    drop_stg_table_before = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        DROP TABLE IF EXISTS stg."tmp_{TARGET_TABLE}_{{{{ data_interval_start.subtract(days = 1).format('YYYY-MM-DD') }}}}"
        """,
    )
    ...
    create_stg_table = SQLExecuteQueryOperator(
        ...
        autocommit=True,
        sql=f"""
        CREATE TABLE stg."tmp_{TARGET_TABLE}_{{{{ data_interval_start.subtract(days = 1).format('YYYY-MM-DD') }}}}" AS
        SELECT
            time::date AS date,
            avg(mag::float)
        FROM
            ods.fct_earthquake
        WHERE
            time::date = '{{{{ data_interval_start.subtract(days = 1).format('YYYY-MM-DD') }}}}'
        GROUP BY 1
        """,
    )
    ...
    delete_from_target_table = SQLExecuteQueryOperator(
        ...
        WHERE date IN
        (
            SELECT date FROM stg."tmp_{TARGET_TABLE}_{{{{ data_interval_start.subtract(days = 1).format('YYYY-MM-DD') }}}}"
        )
        """,
    )
    ...
    insert_into_target_table = SQLExecuteQueryOperator(
        ...
        SELECT * FROM stg."tmp_{TARGET_TABLE}_{{{{ data_interval_start.subtract(days = 1).format('YYYY-MM-DD') }}}}"
        """,
    )
